...
# from models.SwinTransformer import SwinV2_CSPB
...
class GAMAttention(nn.Module):
	def _init_(self, c1, c2, group=True,rate=4):
		super(GAMAttention, self)._init_()
...
#combine Conv and attention to avoid large change of backbone
...
#combine SPPCSPC and attention to avoid large change of backbone
...
class WindowAttention_v2(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        devicefix = torch.device('cuda:0')
        B_, N, C = x.shape
        qkv_bias = None
        if self.q_bias is not None:
            qkv_bias = torch.cat((self.q_bias, torch.zeros_like(self.v_bias, requires_grad=False), self.v_bias))
        qkv = F.linear(input=x.to(devicefix), weight=self.qkv.weight.to(devicefix), bias=qkv_bias.to(devicefix))
        qkv = qkv.reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)

        # cosine attention
        attn = (F.normalize(q, dim=-1).to(devicefix) @ F.normalize(k, dim=-1).transpose(-2, -1).to(devicefix))
        logit_scale = torch.clamp(self.logit_scale.to(devicefix), max=torch.log(torch.tensor(1. / 0.01).to(devicefix)).to(devicefix)).exp().to(devicefix)
        ...
        attn = attn + relative_position_bias.unsqueeze(0).to(devicefix)
        ...
        try:
            x = (attn @ v).transpose(1, 2).reshape(B_, N, C).to(devicefix)
        except:
            x = (attn.half() @ v).transpose(1, 2).reshape(B_, N, C).to(devicefix)
            
        x = self.proj(x.to(devicefix))
        x = self.proj_drop(x)
        return x
...
class SwinTransformerLayer_v2(nn.Module):
    ...
    def forward(self, x):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows.to(devicefix), mask=attn_mask).to(devicefix)  # nW*B, window_size*window_size, C
...
class SwinV2_CSPB(nn.Module):
    ...
    def forward(self, x):
        devicefix = torch.device('cuda:0')
        x1 = self.cv1(x)
        y1 = self.m(x1.to(devicefix))
        y2 = self.cv2(x1)
        return self.cv3(torch.cat((y1, y2).to(devicefix), dim=1))
